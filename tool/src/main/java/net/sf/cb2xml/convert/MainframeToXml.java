            ...
            try {
                ...
                if ("true".equals(element.getAttribute("numeric"))) {
                    ...
                    if ("true".equals(element.getAttribute("signed"))) {
                        String digit = textForNumeric.substring(textForNumeric.length() - 1, textForNumeric.length());
                        digit = SignedNumericMappingTable.decodeMap.get(
                                digit);
                ...
                throw new Exception(String.format("can't parse copybook string on element '%s' and text '%s'", resultElementName, text), e);
            }
